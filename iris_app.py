...
# --- Load built-in data from bezdekIris.data content ---
...
with tab1:
    ...
    if st.button("🔍 Predict Species", use_container_width=True):
        ...
        prob_df = pd.DataFrame({"Species": classes, "Probability": probabilities})
        for _, row in prob_df.iterrows():
            st.progress(float(row["Probability"]),
                        text=f"{SPECIES_EMOJI.get(row['Species'],'')} {row['Species']}: {row['Probability']*100:.1f}%")

    st.markdown("---")
    st.markdown("**Or upload a CSV file** (columns: sepallength, sepalwidth, petallength, petalwidth) to batch predict:")
    uploaded = st.file_uploader("Upload CSV", type=["csv"])
    if uploaded:
        try:
            user_df = pd.read_csv(uploaded)
            preds = model.predict(user_df.values)
...
with tab2:
    ...
    chart_data = df.pivot_table(index="class", values=feature, aggfunc="mean").reset_index()
    chart_data.columns = ["Species", f"Avg {feature}"]
